...
pub async fn create(State(state): State<AppState>, Json(input): Json<CreateTodo>) -> impl IntoResponse {
    ...
    (StatusCode::OK, Json(ResultWrapper { code: 0, message: "ok".to_string(), data: Some(rowid) }))
}


pub async fn update(State(state): State<AppState>, Json(input): Json<UpdateTodo>) -> impl IntoResponse {
    tracing::info!("todo list info: {:?} had been update", input);
    let conn = state.db.lock().unwrap();
    let id = conn.execute(
        "UPDATE todo SET description = $1, completed = $2 WHERE id = $3",
        params![&input.description, &input.completed, &input.id]).map_err(|e| {
    ...
    (StatusCode::OK, Json(ResultWrapper { code: 0, message: "ok".to_string(), data: Some(id) }))
}
...
pub async fn delete(State(state): State<AppState>, Json(id): Json<i32>) -> impl IntoResponse {
    ...
    let conn = state.db.lock().unwrap();
    let id = conn.execute("DELETE FROM todo WHERE id = $1", &[&id]).map_err(|e| {
...
pub async fn list(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let conn = state.db.lock().unwrap();
    let mut stmt = conn.prepare("SELECT * FROM todo").map_err(|e| {
    ...
    (StatusCode::OK, Json(ResultWrapper::<Vec<Todo>> {
        code: 0,
